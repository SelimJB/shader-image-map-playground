...
void main(){
    ...
    vec4 map=texture2D(u_texture_10,uv);
    vec4 colorMap=texture2D(u_texture_11,uv);
    vec4 colorGlowMap=texture2D(u_texture_13,uv);
    vec4 colorBorders=texture2D(u_texture_12,uv);
    
    vec4 cursorPixelColor=texture2D(u_texture_11,u_mouse/u_resolution);
    ...
    if(cursorQuantization==fragmentQuant&&uCurrentQuantization!=0){
        ...
        if(index==0){
            ...
            provinceColor=mix(fragmentColor,map,.85);
        }
    }
    ...
    float d=smoothstep(0.,.3,distance(uv,mousePosition));
